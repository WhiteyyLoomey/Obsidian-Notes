
# Changes: 

Essence (Model Remains Intact)
- You can now only one type of essence each time and every max level is 2. (Ex: Only Strength 1, only Speed 1).

- Essence now gives permanent effect 1, and a secondary ability. (/essence attack) to activate.

Dragon Egg:
Enderman no longer aggro on you.
Please make regeneration 1 work.


## Essence:

Strength 1: Perm Strength 1
Strength 2: Perm Strength 2, Ability: Shield Disable lasts 10 seconds, once succesfully disables or pop a totem, enemy gets stunned for 5 seconds.
Ability Cooldown: 1m

Speed 1: Perm Speed 1
Speed 2: Perm Speed 2, Ability: Slow Enemies around a 10 radius circle for 5 seconds. (Use particles to simulate this effect), (Slowness IV)
Ability Cooldown: 1m

Haste 1: Perm Haste 1
Haste 2: Perm Haste 2, Ability: Applies Efficiency 10, Fortune 10, Unbreaking 10 for 15 seconds.
Ability Cooldown: 1m

Health 1: Permanent Bonus Heart 2
Health 2: Permanent Bonus Heart 5, Ability: Able to steal player's hearts to heal yourself (Ex. I deal 5 hearts to my enemy, I heal 5 hearts.)
Ability Cooldown: 1m

Hero of the Village 1: Permanent Hero of the Village 1
Hero of the Village 2: Permanent Hero of the Village 2, Ability: Gives Hero of the Village 100, 5x XP Gained from all sources, Looting 10 for 15 minutes.
Ability Cooldown: 45m

Ocean 1: Permanent Water Breathing
Ocean 2: Permanent Water Breathing, Dolphin's Grace 1. Ability: Able to use a Riptide Trident any time and Impaling finally works on any mobs under the rain for 30 minutes.
Ability Cooldown: 1h

Fire 1: Permanent Fire Resistance
Fire 2: Permanent Fire Resistance, Able to Swim in Lava. Ability: Fire Aspect 2 now gives a blue fire that bypasses Fire Resistance for 20 seconds.
Ability Cooldown: 1m

Breeze 1: Permanent 0 Fall Damage
Breeze 2: Permanent 0 Fall Damage, Deals 4 Extra Damage (2 Hearts) when you are 5 blocks above your enemy. Ability: Launch everything around a 5 block radius 40 blocks into the sky, disables feather falling)
Ability Cooldown: 1m

Void 1: Permanent 0 Aggro from Ender mobs
Void 2: Permanent 0 Aggro from Ender mobs, Gives levitation right before touching the void (Essentially just make it immune to void damage) Ability: Gives Full Invisibility including armor for 1 minute
Ability Cooldown: 5m

# New Addition:

## Golden Spawner (Haven't made the model yet)
Most expensive item in the game.

When you place it down everytime, it gives you an interface, where you can choose which mob it spawns without needing a spawn egg, and spawns mobs more frequently than a normal spawner. Once you break it, it will reset and you'll have to choose the mob again.


## Scythe (Put this on hold I don't have intentions on actually adding this yet)

I don't have a name for this but essentially.
It has the same attack stats as a netherite axe but the attack cooldown of a netherite sword. Special Ability: Launches a vertical attack around 2 blocks wide, consumes 5 health YOURSELF, but if it lands deals 5 hearts and you yourself gains Absorption 10 and Regeneration 5 for 3 seconds. 
Ability Cooldown: 1m


# Recipe:
Strength Essence:
![[image.png]]

Speed Essence:
![[image-1.png]]
Haste Essence: